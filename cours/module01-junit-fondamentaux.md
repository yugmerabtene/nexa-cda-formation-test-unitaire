...
Autrement dit : au lieu de lancer votre application et de cliquer partout pour vérifier que tout fonctionne, vous écrivez un petit programme qui appelle vos méthodes avec des paramètres connus et vérifie que le résultat est celui attendu.
...
Exemple avec un commentaire (que vous ne mettrez pas en production, car le code doit être auto-documenté) :
...
4. **Améliorer la conception** : un code difficile à tester est souvent un code mal conçu. Si vous n'arrivez pas à tester une méthode, c'est peut-être qu'elle fait trop de choses, ou qu'elle dépend trop de ses voisines. Le test unitaire vous force à écrire du code modulaire.
